...
public class LoginInterceptor extends HandlerInterceptorAdapter {
	 ...
	 * 
	 * // session 객체를 가져옴 HttpSession session = request.getSession();
	 * 
	 * // login처리를 담당하는 사용자 정보를 담고 있는 객체를 가져옴 MemberVO mVO = (MemberVO)
	 * session.getAttribute("loginUser");
	 * 
	 * if (mVO == null) { // 로그인이 안되어 있는 상태임으로 로그인 폼으로 다시 돌려보냄(redirect)
